...
bool RendererImpl::InitPostureRendering() {
  ...
  {  // Prepares bone mesh.
    const ozz::math::Float3 pos[6] = {
        ozz::math::Float3(1.f, 0.f, 0.f),     ozz::math::Float3(kInter, .1f, .1f),
        ozz::math::Float3(kInter, .1f, -.1f), ozz::math::Float3(kInter, -.1f, -.1f),
        ozz::math::Float3(kInter, -.1f, .1f), ozz::math::Float3(0.f, 0.f, 0.f)};
...
bool RendererImpl::InitCheckeredTexture() {
  ...
  uint8_t* pixels = reinterpret_cast<uint8_t*>(
      ozz::memory::default_allocator()->Allocate(buffer_size, 16));
...
bool RendererImpl::DrawBoxShaded(
  ...
  const ozz::math::Float3 pos[8] = {
      ozz::math::Float3(_box.min.x, _box.min.y, _box.min.z),
  ...
  const ozz::math::Float3 normals[6] = {
      ozz::math::Float3(-1, 0, 0), ozz::math::Float3(1, 0, 0),  ozz::math::Float3(0, -1, 0),
      ozz::math::Float3(0, 1, 0),  ozz::math::Float3(0, 0, -1), ozz::math::Float3(0, 0, 1)};
...
bool RendererImpl::DrawLines(ozz::span<const ozz::math::Float3> _vertices,
                             const Color& _color,
                             const ozz::math::Float4x4& _transform) {
  if (_vertices.size() < 2) {
    return true;
...
bool RendererImpl::DrawLineStrip(ozz::span<const ozz::math::Float3> _vertices,
                                 const Color& _color,
                                 const ozz::math::Float4x4& _transform) {
  if (_vertices.size() < 2) {
    return true;
...
bool RendererImpl::DrawVectors(ozz::span<const float> _positions,
  ...
  if (ozz::PointerStride(_positions.begin(), _positions_stride * _num_vectors) >
          _positions.end() ||
      ozz::PointerStride(_directions.begin(), _directions_stride * _num_vectors) >
          _directions.end()) {
    return false;
  }
  ...
  for (int i = 0; i < _num_vectors; ++i) {
    const float* position =
        ozz::PointerStride(_positions.data(), _positions_stride * i);
    v.pos[0] = position[0];
    v.pos[1] = position[1];
    v.pos[2] = position[2];
    im.PushVertex(v);
    ...
    const float* direction =
        ozz::PointerStride(_directions.data(), _directions_stride * i);
...
bool RendererImpl::DrawBinormals(
  ...
  if (ozz::PointerStride(_positions.begin(), _positions_stride * _num_vectors) >
          _positions.end() ||
      ozz::PointerStride(_normals.begin(), _normals_stride * _num_vectors) >
          _normals.end() ||
          ozz::PointerStride(_tangents.begin(), _tangents_stride * _num_vectors) >
          _tangents.end() ||
          ozz::PointerStride(_handenesses.begin(), _handenesses_stride * _num_vectors) >
          _handenesses.end()) {
    return false;
  }
  ...
  for (int i = 0; i < _num_vectors; ++i) {
    ...
    ozz::PointerStride(_tangents.data(), _tangents_stride * i);
    const ozz::math::Float3 tangent(p_tangent[0], p_tangent[1], p_tangent[2]);
    const float* p_handedness =
    ozz::PointerStride(_handenesses.data(), _handenesses_stride * i);
  ...
  if (_options.normals) {
    for (size_t i = 0; i < _mesh.parts.size(); ++i) {
      ...
      DrawVectors(make_span(part.positions),
                  ...
                  part.vertex_count(), .03f, myns::kGreen, _transform);
    }
  }
    ...
    for (size_t i = 0; i < _mesh.parts.size(); ++i) {
      ...
      if (part.normals.size() != 0) {
        DrawVectors(make_span(part.positions),
                    ...
                    part.vertex_count(), .03f, myns::kRed, _transform);
      }
    }
  }
    ...
    for (size_t i = 0; i < _mesh.parts.size(); ++i) {
      ...
      if (part.normals.size() != 0 && part.tangents.size() != 0) {
        DrawBinormals(
            ...
            Mesh::Part::kTangentsCpnts * sizeof(float),
            part.vertex_count(), .03f, myns::kBlue, _transform);
      }
    }
  }
...
bool RendererImpl::DrawSkinnedMesh(
    const myns::Mesh& _mesh, const ozz::span<ozz::math::Float4x4> _skinning_matrices,
  ...
  for (size_t i = 0; i < _mesh.parts.size(); ++i) {
    ...
    if (part.normals.size() / Mesh::Part::kNormalsCpnts ==
        part_vertex_count) {
    ...
    if (part.tangents.size() / Mesh::Part::kTangentsCpnts ==
        part_vertex_count) {
      ...
      skinning_job.in_tangents_stride =
          sizeof(float) * Mesh::Part::kTangentsCpnts;
    ...
    if (_options.normals && skinning_job.out_normals.size() > 0) {
      DrawVectors(skinning_job.out_positions, skinning_job.out_positions_stride,
                  ...
                  skinning_job.vertex_count, .03f, myns::kGreen,
                  _transform);
    }
    ...
    if (_options.tangents && skinning_job.out_tangents.size() > 0) {
      DrawVectors(skinning_job.out_positions, skinning_job.out_positions_stride,
                  ...
                  skinning_job.vertex_count, .03f, myns::kRed,
                  _transform);
    }
  ...
  if (_options.vertices) {
    myns::Color color = myns::kWhite;
    const ozz::span<const float> vertices{
