...
namespace Engine{
	...
	bool RendererImpl::DrawMesh(const AnimatedMesh& _mesh, const ozz::math::Float4x4& _transform, MaterialHandle _material, const Options& _options)
	{
		...
		const GLsizei positions_offset = 0;
		const GLsizei positions_stride = sizeof(float) * AnimatedMesh::Part::kPositionsCpnts;
		const GLsizei positions_size   = vertex_count * positions_stride;
		const GLsizei normals_offset   = positions_offset + positions_size;
		const GLsizei normals_stride   = sizeof(float) * AnimatedMesh::Part::kNormalsCpnts;
		...
		const GLsizei colors_stride = sizeof(uint8_t) * AnimatedMesh::Part::kColorsCpnts;
		...
		const GLsizei uvs_stride = _options.texture ? sizeof(float) * AnimatedMesh::Part::kUVsCpnts : 0;
		...
		for (const auto& part : _mesh.parts) {
			...
			// Handles normals.
			const size_t part_normal_count = part.normals.size() / AnimatedMesh::Part::kNormalsCpnts;
			...
			// Handles colors.
			const size_t part_color_count = part.colors.size() / AnimatedMesh::Part::kColorsCpnts;
			...
			// Handles uvs.
			if (_options.texture) {
				const size_t part_uvs_count = part.uvs.size() / AnimatedMesh::Part::kUVsCpnts;
		...
		if (_options.triangles) {
			...
			GL(BindBuffer(GL_ELEMENT_ARRAY_BUFFER, dynamic_index_bo_));
			const AnimatedMesh::TriangleIndices& indices = _mesh.triangle_indices;
			...
			// Draws the mesh.
			static_assert(sizeof(AnimatedMesh::TriangleIndices::value_type) == 2, "Expects 2 bytes indices.");
		...
		if (_options.vertices) {
			for (size_t i = 0; i < _mesh.parts.size(); ++i) {
				const AnimatedMesh::Part& part  = _mesh.parts[i];
		...
		if (_options.normals) {
			for (size_t i = 0; i < _mesh.parts.size(); ++i) {
				const AnimatedMesh::Part& part = _mesh.parts[i];
				DrawVectors(make_span(part.positions), AnimatedMesh::Part::kPositionsCpnts * sizeof(float), make_span(part.normals), AnimatedMesh::Part::kNormalsCpnts * sizeof(float), part.vertex_count(), .03f, Engine::kGreen, _transform);
			}
		}
		...
		if (_options.tangents) {
			for (size_t i = 0; i < _mesh.parts.size(); ++i) {
				...
				if (part.normals.size() != 0) {
					DrawVectors(make_span(part.positions), AnimatedMesh::Part::kPositionsCpnts * sizeof(float), make_span(part.tangents), AnimatedMesh::Part::kTangentsCpnts * sizeof(float), part.vertex_count(), .03f, Engine::kRed, _transform);
				}
			}
		}
			...
			for (size_t i = 0; i < _mesh.parts.size(); ++i) {
				...
				if (part.normals.size() != 0 && part.tangents.size() != 0) {
					DrawBinormals(make_span(part.positions),
					              AnimatedMesh::Part::kPositionsCpnts * sizeof(float),
					              make_span(part.normals),
					              AnimatedMesh::Part::kNormalsCpnts * sizeof(float),
					              make_span(part.tangents),
					              AnimatedMesh::Part::kTangentsCpnts * sizeof(float),
	...
	{
		ZoneScopedN("DrawSkinnedMesh (GBuffer)");
		...
		{
			ZoneScopedN("GBuffer Skin Scratch Alloc");
			vbo_map = scratch_buffer_.Resize(vbo_size);
		}
		...
		{
			ZoneScopedN("GBuffer CPU Skinning Jobs");
			for (size_t i = 0; i < _mesh.parts.size(); ++i) {
				const AnimatedMesh::Part& part = _mesh.parts[i];
				...
				if (_options.colors && part_vertex_count == part.colors.size() / AnimatedMesh::Part::kColorsCpnts) {
					// Optimal path used when the right number of colors is provided.
		...
		if (_options.triangles) {
			{
				ZoneScopedN("GBuffer Upload Skinned VBO/IBO");
	...
	bool RendererImpl::DrawSkinnedMeshMousePicking(glm::vec3 entityColor, const AnimatedMesh& _mesh, ozz::span<ozz::math::Float4x4> _skinning_matrices, const ozz::math::Float4x4& _transform)
	{
		ZoneScopedN("DrawSkinnedMeshMousePicking");
		const int vertex_count = _mesh.vertex_count();
		...
		void*         vbo_map  = nullptr;
		{
			ZoneScopedN("MousePick Skin Scratch Alloc");
			vbo_map = scratch_buffer_.Resize(vbo_size);
		}
		...
		size_t processed_vertex_count = 0;
		{
			ZoneScopedN("MousePick CPU Skinning Jobs");
			for (size_t i = 0; i < _mesh.parts.size(); ++i) {
				const AnimatedMesh::Part& part = _mesh.parts[i];
		...
		{
			ZoneScopedN("MousePick Upload Skinned VBO/IBO");
			// Updates dynamic vertex buffer with skinned data.
			GL(BindBuffer(GL_ARRAY_BUFFER, dynamic_array_bo_));
			GL(BufferData(GL_ARRAY_BUFFER, vbo_size, nullptr, GL_STREAM_DRAW));
			GL(BufferSubData(GL_ARRAY_BUFFER, 0, vbo_size, vbo_map));
	...
	bool RendererImpl::DrawSkinnedMeshShadows(Engine::Shader* shadowShader, const AnimatedMesh& _mesh, ozz::span<ozz::math::Float4x4> _skinning_matrices, const ozz::math::Float4x4& _transform)
	{
		ZoneScopedN("DrawSkinnedMeshShadows");
		const int vertex_count = _mesh.vertex_count();
		...
		void*         vbo_map  = nullptr;
		{
			ZoneScopedN("Shadow Skin Scratch Alloc");
			vbo_map = scratch_buffer_.Resize(vbo_size);
		}
		...
		size_t processed_vertex_count = 0;
		{
			ZoneScopedN("Shadow CPU Skinning Jobs");
			for (size_t i = 0; i < _mesh.parts.size(); ++i) {
				const AnimatedMesh::Part& part = _mesh.parts[i];
		...
		{
			ZoneScopedN("Shadow Upload Skinned VBO/IBO");
			// Updates dynamic vertex buffer with skinned data.
			GL(BindBuffer(GL_ARRAY_BUFFER, dynamic_array_bo_));
			GL(BufferData(GL_ARRAY_BUFFER, vbo_size, nullptr, GL_STREAM_DRAW));
			GL(BufferSubData(GL_ARRAY_BUFFER, 0, vbo_size, vbo_map));
